...
library ArcadeHookLib {
    ...
    /// @dev Creator's share of every harvested post-graduation LP fee, PUMP and
    ///      CLANKER alike (80% creator / 20% treasury). The only definition: the
    ///      hook itself no longer splits anything per swap.
    ...
    uint24 internal constant PUMP_POOL_FEE = 10_000; // ArcadeHook.PUMP_POOL_FEE, 1% in pips
    ...
    // -------------------------------------------------------------------
    // Fee routing (called from the locked-position harvest, kind 2)
    // -------------------------------------------------------------------
    ...
    function createLaunchCore(
        ...
        // The pool's static native LP fee, fixed for life HERE so the PoolId is
        // the same before and after graduation: the CLANKER tier, or the 1%
        // PUMP_POOL_FEE for a PUMP token whose pool only opens when it graduates.
        poolFeeOf[tokenAddr] =
            p.mode == uint8(ArcadeHook.LaunchMode.CLANKER) ? uint24(feeTierBps) * 100 : PUMP_POOL_FEE;
        ...
        // Resolved BEFORE the state writes below because it decides creator2.
        ...
        if (
            ...
            // A handle launch and creator2 are mutually exclusive, exactly as in
            // createRwaLaunch. distributeFee pays creator2 BEFORE the escrow, so
            // creator2Bps = 10_000 emptied the handle's share while the launch
            // still emitted FeeAttributedToHandle: "fees go to the handle" with
            // nothing going there (audit 2026-09-19 L4). The handle wins; the
            // split is dropped rather than reverted so a caller that always
            // sends both keeps launching.
            p.creator2 = address(0);
            p.creator2Bps = 0;
        }
    ...
    function graduate(
        ...
        // Record the full-range seed as the token's harvestable locked position
        // so collectFees addresses a graduated PUMP pool exactly like a CLANKER
        // one (the kind-2 unlock reads clankerPos): the 1% native LP fee accrues
        // there from the first post-grad swap. Written before the seed unlock,
        // as launchDirect does. launchedAt is informational for PUMP (its
        // anti-sniper ran on the curve; afterSwap applies no buy cap to it).
        (int24 minT, int24 maxT) = ArcadeV4Math.fullRange(key.tickSpacing);
        clankerPos[token] =
            ArcadeHook.ClankerPos({tickLower: minT, tickUpper: maxT, seeded: true, launchedAt: uint64(block.timestamp)});
    ...
    /// @dev Realise the locked position's accrued LP fee (a zero-delta modify on
    ///      the range clankerPos records: the CLANKER / RWA single-sided seed or
    ///      the PUMP full-range graduation seed) and split it 80/20 side by side.
    ///      THE fee path for PUMP and CLANKER: collectFees (kind 2) and the
    ///      graveyard sweep (kind 3, before the principal moves) both run it, so
    ///      the split is defined once (audit 2026-09-18 LOW-2).
    ...
    ///      RWA single-sided seed (1), the locked-position fee harvest (2, the
    ///      CLANKER seed or the PUMP graduation seed, both in clankerPos), the
    ///      graveyard sweep (3) or the CLANKER dev-buy (4). Runs under
    ...
    function unlockCallback(
        ...
        // kind 2 = locked-position fee harvest: collect (liquidityDelta 0) on
        // the range clankerPos records, the CLANKER single-sided seed or the
        // PUMP full-range graduation seed, then split 80/20 side by side.
        if (kind == 2) {
            _harvestLocked(pm, treasury, key, quote, token, clankerPos, curveStates, feeOwners, pending);
            return "";
        }

        // kind 3 = graveyard sweep: harvest the fee accrued since the last
        // collect (PUMP / CLANKER: the creator's 80/20, never the treasury's),
        // then remove the FULL liquidity of the hook-owned locked position of a
        // DEAD pool and send BOTH withdrawn sides of the principal to the
        ...
        if (kind == 3) {
            ...
            uint256 tokenOut;
            if (liq > 0) {
                // The fee accrued since the last harvest belongs 80/20 to the
                // creator / treasury, not to the treasury alone: realise it FIRST
                // through the very collect + split collectFees runs. The zero-delta
                // modify checkpoints the position, so the removal below returns the
                // principal alone (audit 2026-09-18 LOW-2). PUMP and CLANKER only:
                // an RWA fee has a 3-way split with a dividend leg, and its
                // harvest is a separate unlock the sweep does not run.
                if (gMode == uint8(0) || gMode == uint8(1)) {
                    _harvestLocked(pm, treasury, key, quote, token, clankerPos, curveStates, feeOwners, pending);
                }
                ...
                // Removal returns positive deltas (the principal; for RWA also the
                // never-harvested fee) owed to the hook. Take BOTH sides to the
                // treasury, pull-safe.
