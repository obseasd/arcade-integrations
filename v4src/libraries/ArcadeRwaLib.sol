...
library ArcadeRwaLib {
    ...
    /// @dev Resolve the RWA launch token (the side with a nonzero quoteAssetOf)
    ///      and its quote currency for a pool key. Only valid on an RWA pool.
    function _tokenQuote(PoolKey calldata key, mapping(address => address) storage quoteAssetOf)
        internal
        view
        returns (address token, Currency quote)
    {
        address c0 = Currency.unwrap(key.currency0);
        token = quoteAssetOf[c0] != address(0) ? c0 : Currency.unwrap(key.currency1);
        quote = Currency.wrap(quoteAssetOf[token]);
    }
    ...
    function _splitAndRoute(
        ...
        if (holdersAmt > 0) {
            pm.take(quote, ctx.distributor, holdersAmt);
